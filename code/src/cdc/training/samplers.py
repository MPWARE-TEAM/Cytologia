...
def get_sampler(sampler_name, ds=None):
    sampler = None
    if sampler_name is not None:
        if sampler_name == "ImbalancedDatasetSampler":
            sampler = ImbalancedDatasetSampler(ds, ds.df["class"].values)
        else:
...
def get_batch_sampler(conf, ds=None, debug=False):
    batch_sampler = None
    if conf.batch_sampler is not None:
        if (conf.batch_sampler == "fixed") or (conf.batch_sampler == "random"):
            # List of indexes per class
            # class_idxs = ds.df.reset_index().groupby("class")["index"].agg(list).reset_index()["index"].values.tolist()
            tmp = ds.df.copy()
            tmp["index"] = range(0, len(tmp))
            class_idxs = tmp.groupby("class")["index"].agg(list).reset_index()["index"].values.tolist()
            if debug:
                # print("DS:", len(ds), "BS:", conf.batch_size, "n_batches:", len(ds) // conf.batch_size)
                for j, i in enumerate(class_idxs):
                    print(j, "=", len(i))
            batch_sampler = SamplerFactory().get(
                class_idxs=class_idxs,
                batch_size=conf.batch_size,
                n_batches=len(ds) // conf.batch_size,
                alpha=conf.batch_sampler_alpha,
                kind=conf.batch_sampler
            )
        else:
            raise NotImplementedError
...
# https://github.com/khornlund/pytorch-balanced-sampler/tree/master
